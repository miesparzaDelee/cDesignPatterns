...
def generate_typst_master(structure_items):
    ...
    with open(typst_path, 'w', encoding='utf-8') as f:
        ...
        f.write('#import "@preview/cmarker:0.1.1"\n')
        f.write('#import "/book/template.typ": project\n\n') 
        ...
        for item in structure_items:
            ...
            # Only add pagebreak if it is a MAIN chapter (not a section)
            # and it's not the very first chapter of the book.
            if not is_section:
                if not first_chapter:
                    f.write('#pagebreak(weak: true)\n')
                first_chapter = False
            else:
                f.write('// Section (No page break)\n')

            f.write(f'#cmarker.render(read("{filename}"))\n\n')
