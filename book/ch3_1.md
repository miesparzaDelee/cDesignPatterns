...
- **Reusability**:  
  ...
  *Without Patterns (Hardcoded)*:
  ...
  ```c
  void generic_init() {
      // Hardcoded to specific hardware (e.g., ATmega328P)
      PORTB |= (1 << 5); 
      TCCR0A = 0x02;
  }
  ```

  *With Patterns (Hardware Abstraction)*:

  ```c
  void generic_init(gpio_t* status_led) {
      // Reusable driver interface
      gpio_write(status_led, HIGH);
      timer_start(TIMER_0, MODE_CTC);
  }
  ```

  In the second example, the logic remains valid whether you are running on an AVR, an ARM Cortex-M, or even a PC simulation. The `gpio_write` abstraction hides the hardware details.
...
- **Scalability**:  
  ...
  *Without Patterns (Monster Switch)*:
  ...
  ```c
  void process_event(event_t e) {
      switch(e) {
          case EVT_BTN_PRESS: handle_btn(); break;
          case EVT_TIMER:     handle_timer(); break;
          // ... 100 more cases ...
          case EVT_USB:       handle_usb(); break;
          // Adding a new event requires editing this massive function
      }
  }
  ```
  
  *With Patterns (Table-Driven / Command)*:
  ...
  // Adding a new feature is just adding a row to this table
  static const event_handler_t handlers[] = {
      {EVT_BTN_PRESS, handle_btn},
      {EVT_TIMER,     handle_timer},
      {EVT_USB,       handle_usb},
  };
  ```
