...
{{ file "companion_code/ch2_patterns/include/rectangle.h" type="typedef" name="rectangle_t" }}
...
{{ file "companion_code/ch2_patterns/include/rectangle.h" type="function" name="rect_init" }}

{{ file "companion_code/ch2_patterns/include/rectangle.h" type="function" name="rect_get_area" }}
...
{{ file "companion_code/ch2_patterns/src/rectangle.c" type="function" name="rect_init" }}

{{ file "companion_code/ch2_patterns/src/rectangle.c" type="function" name="rect_get_area" }}
...
Below is an example of how to use the module to manage multiple independent objects:

```c
#include "rectangle.h"

// 1. Declare instances (memory allocation)
rectangle_t small_rect;
rectangle_t big_rect;

int main() {

    // 2. Initialize them with different states
    rect_init(&small_rect, 5, 10);
    rect_init(&big_rect,   100, 200);

    // 3. Operate on them independently
    uint32_t area1 = rect_get_area(&small_rect); // Returns 50
    uint32_t area2 = rect_get_area(&big_rect);   // Returns 20000

    printf("Small Area: %d, Big Area: %d\n", area1, area2);

    return 0;
}
```

In this example, `small_rect` and `big_rect` are completely independent. Changing the width of one would not affect the other.
This is the power of the Object Pattern: **Encapsulation of state allows for multiple instances of the same logic.**
