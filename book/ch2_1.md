...
<aside class="convention" id="FO-01" data-title="Library Organization">
...
├── scripts/      # Build scripts   (see FO-02)
├── build/        # Build files     (see FO-02)
└── dist/         # Build artifacts (see FO-02)
```

</aside>

<aside class="convention" id="FO-02" data-title="Library Build Layout">

**Standard**: Every library keeps checked-in build definitions under `build/`, project-specific build helper scripts under `scripts/`, and generated release artifacts under `dist/`.

**Rationale**: A fixed build layout keeps CI, IDE tasks, and local builds aligned.

**Detail**: `build/` contains the build system entry points, such as `Makefile`, `CMakeLists.txt`, or other build configuration files. Generated intermediate files go under `build/out/` and must be ignored by git. `scripts/` contains helper scripts that developers and CI can run in the same way. `dist/` contains final generated artifacts, such as `.a`, `.so`, or `.dll` files, and must also be ignored by git.

**Note**: Unit test runner scripts may also live in this area, such as `run_tests.sh` and a Windows wrapper that invokes it through WSL. The goal is that build and verification entry points are easy to find.
...
my_library/
├── build/
│   ├── out/              # Intermediate build files, ignored by git
│   │   └── ...
...
**Host vs Target**: Note that the library build in `build/` uses the **target compiler** (e.g., `arm-none-eabi-gcc`), as it produces artifacts meant to run on the embedded device. Unit tests, on the other hand, always use the **host compiler** (e.g., `gcc` on your development machine) since they run locally. These are separate build configurations: the library `build/Makefile` cross-compiles for the target, while `tests/build/Makefile` compiles for the host.

</aside>

<aside class="convention" id="FO-03" data-title="Library Independence">
...
1. **Development mode (source)**: Include the library as a git submodule and add its `inc/` folder to your project's include path. The consuming project compiles the library's sources directly alongside its own. Use this during active development, when modifying the library, or when building an application-layer library that wraps several modules together.
2. **Release mode (pre-compiled)**: Compile the library separately into a static archive (`.a`) in `dist/`, then distribute the archive along with the public headers. The consuming project links against the artifact without accessing the source. Use this for stable dependencies or when distributing a library to a customer without exposing implementation details.

In practice, most embedded teams work primarily in development mode. The `build/` + `dist/` structure supports both workflows without any change to the library's code or folder layout.
...
<aside class="convention" id="FO-04" data-title="Module Structure">
...
│   └── uart/
│       └── uart_defs.h # Supplemental header
└── src/
...
<aside class="convention" id="FO-05" data-title="Sub-module Splitting">
...
// Query function
uart_status_t uart_getStatus(void);
...
<aside class="convention" id="NC-02" data-title="Function Semantic Naming">
...
**Detail**: Common semantic suffixes include lifecycle management (`_init`, `_deinit`, `_start`, `_stop`), periodic operations (`_tasks`, `_update`), data processing (`_process`), and event handling (`_handle`). Apply these patterns consistently where they fit, but don't force them where they don't naturally apply. The goal is recognition, not rigid compliance.
...
void sensor_tasks(sensor_t* self);                        
...
void filter_process(filter_t* self, float* data);
```

</aside>

<aside class="convention" id="NC-03" data-title="Typedef Suffix">
...
<aside class="convention" id="NC-04" data-title="Handle Prefix">
...
<aside class="convention" id="NC-05" data-title="Macro Naming">
...
```c
/* uart.h - Header file 
   (public macros with module prefix)
*/
#ifndef UART_H
#define UART_H
...
/* uart.c - Source file 
   (local macros can use simpler names)
*/
...
<aside class="convention" id="NC-06" data-title="Private Naming">
...
<aside class="convention" id="CS-01" data-title="Design First">
...
<aside class="convention" id="CS-02" data-title="Architecture Decision Records">
...
<aside class="convention" id="CS-03" data-title="Header Content">
...
<aside class="convention" id="CS-04" data-title="Header Organization">
...
<aside class="convention" id="CS-05" data-title="Configuration Structs">
...
<aside class="convention" id="CS-06" data-title="Discriminated Unions for Variants">
...
<aside class="convention" id="CS-08" data-title="Include Order">
...
<aside class="convention" id="CS-09" data-title="Source Organization">
...
<aside class="convention" id="DO-01" data-title="Public API Documentation">
...
<aside class="convention" id="DO-02" data-title="Internal Documentation">
