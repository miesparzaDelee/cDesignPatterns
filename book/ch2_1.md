...
<aside class="convention" id="FO-02" data-title="Library Build Artifacts">
...
**Note**: We also keep unit test runner scripts here (for example, a `run_tests.sh` and a Windows wrapper that invokes it through WSL). The goal is that all build and verification entry points live in one predictable place.
